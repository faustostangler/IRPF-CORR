...
def apportion_fees(note: BrokerageNote) -> List[Trade]:
    ...
    for trade in note.trades:
        ...
        new_trade.unit_price = new_trade.unit_price * note.exchange_rate
        new_trade.allocated_fees = (allocated_fee * note.exchange_rate).quantize(
...
def calculate_average_cost(position: Position, trade: Trade) -> Position:
    ...
    if trade.direction == "BUY":
        current_invested = (
            Decimal(str(new_position.quantity)) * new_position.average_cost
        )
        trade_cost = (
            Decimal(str(trade.quantity)) * trade.unit_price
        ) + trade.allocated_fees
        ...
        if new_position.quantity > 0:
            ...
            new_position.average_cost = avg_cost.quantize(
                Decimal("0.0001"), rounding=ROUND_HALF_UP
            )

    elif trade.direction == "SELL":
        new_position.quantity -= trade.quantity
        new_position.total_invested = (
            Decimal(str(new_position.quantity)) * new_position.average_cost
        )
